...
void GenerateCircleNDCCoefficientes(float xCenter, float yCenter, float zCenter, float radius, int numberOfVertices, GLfloat* NDC_coefficients);
...
GLuint BuildTriangles()
{
    ...
    GenerateCircleNDCCoefficientes(0.0f, 0.0f, 0.0f, 0.7f, numberOfVertices, NDC_coefficients);
    ...
    // Gera os índices para o triangle fan
    GLubyte indices[numberOfVertices + 1];
    for (int i = 0; i < numberOfVertices; ++i)  // Loop que define a topologia de todos itens menos o último
        indices[i] = i;

    indices[numberOfVertices] = indices[1];     // Adição do segundo vértice na última posição para fechar o círculo (o primeiro vértice é o centro)
...
void GenerateColorCoefficients(GLfloat  color_coefficients[68], const int& numberOfVertices)
{
    // Gera as cores do vértice central como a primeira linha da matriz de coeficientes
    color_coefficients[0] = 1.0f;
    color_coefficients[1] = 0.0f;
    color_coefficients[2] = 0.0f;
    color_coefficients[3] = 0.0f;

    // Gera as cores dos vértices restantes a partir da segunda linha da matriz de coeficientes
...
void GenerateCircleNDCCoefficientes(float xCenter, float yCenter, float zCenter, float radius, int numberOfVertices, GLfloat* NDC_coefficients)
{
    ...
    // Gera as coordenadas do vértice central como a primeira linha da matriz de coeficientes
    NDC_coefficients[0] = xCenter;
    NDC_coefficients[1] = yCenter;
    NDC_coefficients[2] = zCenter;
    NDC_coefficients[3] = 1.0f;         // Valor padrão para ponto de coordenadas homogêneas

    // Gera as coordenadas dos vértices restantes a partir da segunda linha da matriz de coeficientes
    for (int i = 1; i < numberOfVertices; i++)
    {
        float angleRadians = (i - 1) * angleIncrement;
