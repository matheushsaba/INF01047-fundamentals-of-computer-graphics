...
int main()
{
    ...
    GLFWwindow* window;
    window = glfwCreateWindow(800, 800, "INF01047 - 00228729 - Matheus Henrique Sabadin", NULL, NULL);
    if (!window)
    {
        glfwTerminate();
    ...
    while (!glfwWindowShouldClose(window))
    {
            ...
            PushMatrix(model); // Guardamos matriz model atual na pilha
                ...
                PushMatrix(model); // Guardamos matriz model atual na pilha
                    ...
                    PushMatrix(model); // Guardamos matriz model atual na pilha
                        ...
                        // Código novo =====================
                        PushMatrix(model); // Guardamos matriz model atual na pilha
                            model = model * Matrix_Translate(0.0f, -1.08f, 0.0f); // Atualizamos matriz model (multiplicação à direita) com a translação da mão direita
                            PushMatrix(model); // Guardamos matriz model atual na pilha
                                model = model * Matrix_Scale(1.0f, 0.15f, 1.0f); // Atualizamos matriz model (multiplicação à direita) com um escalamento da mão direita
                                glUniformMatrix4fv(model_uniform, 1, GL_FALSE, glm::value_ptr(model)); // Enviamos matriz model atual para a GPU
                                DrawCube(render_as_black_uniform); // #### ANTEBRAÇO DIREITO // Desenhamos a mão direita
                            PopMatrix(model); // Tiramos da pilha a matriz model guardada anteriormente
                        PopMatrix(model); // Tiramos da pilha a matriz model guardada anteriormente
                        // Código novo =====================
        ...
        // ===========================
        PushMatrix(model); // Guardamos matriz model atual na pilha
            model = model * Matrix_Translate(0.55f, 0.0f, 0.0f); // Atualizamos matriz model (multiplicação à esquerda) com uma translação para o braço esquerdo
            PushMatrix(model); // Guardamos matriz model atual na pilha
                model = model // Atualizamos matriz model (multiplicação à esquerda) com a rotação do braço esquerdo
                      * Matrix_Rotate_Z(-g_AngleZ)  // TERCEIRO rotação Z de Euler
                      * Matrix_Rotate_Y(g_AngleY)  // SEGUNDO rotação Y de Euler
                      * Matrix_Rotate_X(g_AngleX); // PRIMEIRO rotação X de Euler
                PushMatrix(model); // Guardamos matriz model atual na pilha
                    model = model * Matrix_Scale(0.2f, 0.6f, 0.2f); // Atualizamos matriz model (multiplicação à esquerda) com um escalamento do braço esquerdo
                    glUniformMatrix4fv(model_uniform, 1, GL_FALSE, glm::value_ptr(model)); // Enviamos matriz model atual para a GPU
                    DrawCube(render_as_black_uniform); // #### BRAÇO ESQUERDO // Desenhamos o braço esquerdo
                PopMatrix(model); // Tiramos da pilha a matriz model guardada anteriormente
                PushMatrix(model); // Guardamos matriz model atual na pilha
                    model = model * Matrix_Translate(0.0f, -0.65f, 0.0f); // Atualizamos matriz model (multiplicação à esquerda) com a translação do antebraço esquerdo
                    model = model // Atualizamos matriz model (multiplicação à esquerda) com a rotação do antebraço esquerdo
                          * Matrix_Rotate_Z(-g_ForearmAngleZ)  // SEGUNDO rotação Z de Euler
                          * Matrix_Rotate_X(g_ForearmAngleX); // PRIMEIRO rotação X de Euler
                    PushMatrix(model); // Guardamos matriz model atual na pilha
                        model = model * Matrix_Scale(0.2f, 0.6f, 0.2f); // Atualizamos matriz model (multiplicação à esquerda) com um escalamento do antebraço esquerdo
                        glUniformMatrix4fv(model_uniform, 1, GL_FALSE, glm::value_ptr(model)); // Enviamos matriz model atual para a GPU
                        DrawCube(render_as_black_uniform); // #### ANTEBRAÇO ESQUERDO // Desenhamos o antebraço esquerdo
                        PushMatrix(model); // Guardamos matriz model atual na pilha
                            model = model * Matrix_Translate(0.0f, -1.08f, 0.0f); // Atualizamos matriz model (multiplicação à esquerda) com a translação da mão esquerda
                            PushMatrix(model); // Guardamos matriz model atual na pilha
                                model = model * Matrix_Scale(1.0f, 0.15f, 1.0f); // Atualizamos matriz model (multiplicação à esquerda) com um escalamento da mão esquerda
                                glUniformMatrix4fv(model_uniform, 1, GL_FALSE, glm::value_ptr(model)); // Enviamos matriz model atual para a GPU
                                DrawCube(render_as_black_uniform); // #### ANTEBRAÇO ESQUERDO // Desenhamos a mão esquerda
                            PopMatrix(model); // Tiramos da pilha a matriz model guardada anteriormente
                        PopMatrix(model); // Tiramos da pilha a matriz model guardada anteriormente
                    PopMatrix(model); // Tiramos da pilha a matriz model guardada anteriormente
                PopMatrix(model); // Tiramos da pilha a matriz model guardada anteriormente
        ...
        PushMatrix(model); // Guardamos matriz model atual na pilha
            ...
            PushMatrix(model); // Guardamos matriz model atual na pilha
                PushMatrix(model); // Guardamos matriz model atual na pilha
                    model = model * Matrix_Scale(0.3f, 0.7f, 0.28f); // Atualizamos matriz model (multiplicação à direita) com um escalamento da perna direita
                    glUniformMatrix4fv(model_uniform, 1, GL_FALSE, glm::value_ptr(model)); // Enviamos matriz model atual para a GPU
                    DrawCube(render_as_black_uniform); // #### BRAÇO DIREITO // Desenhamos a perna direita
                PopMatrix(model); // Tiramos da pilha a matriz model guardada anteriormente
                PushMatrix(model); // Guardamos matriz model atual na pilha
                    model = model * Matrix_Translate(0.0f, -0.75f, 0.0f); // Atualizamos matriz model (multiplicação à direita) com a translação da canela direita
                    PushMatrix(model); // Guardamos matriz model atual na pilha
                        ...
                        PushMatrix(model); // Guardamos matriz model atual na pilha
                            model = model * Matrix_Translate(0.0f, -1.08f, 0.5f); // Atualizamos matriz model (multiplicação à direita) com a translação do pé direito
                            PushMatrix(model); // Guardamos matriz model atual na pilha
                                model = model * Matrix_Scale(0.8f, 0.15f, 2.1f); // Atualizamos matriz model (multiplicação à direita) com um escalamento do pé direito
                                glUniformMatrix4fv(model_uniform, 1, GL_FALSE, glm::value_ptr(model)); // Enviamos matriz model atual para a GPU
        ...
        // ===========================
        PushMatrix(model); // Guardamos matriz model atual na pilha
            model = model * Matrix_Translate(0.2f, -1.05f, 0.0f); // Atualizamos matriz model (multiplicação à esquerda) com uma translação para a perna esquerda
            PushMatrix(model); // Guardamos matriz model atual na pilha
                PushMatrix(model); // Guardamos matriz model atual na pilha
                    model = model * Matrix_Scale(0.3f, 0.7f, 0.28f); // Atualizamos matriz model (multiplicação à esquerda) com um escalamento da perna esquerda
                    glUniformMatrix4fv(model_uniform, 1, GL_FALSE, glm::value_ptr(model)); // Enviamos matriz model atual para a GPU
                    DrawCube(render_as_black_uniform); // #### BRAÇO ESQUERDO // Desenhamos a perna esquerda
                PopMatrix(model); // Tiramos da pilha a matriz model guardada anteriormente
                PushMatrix(model); // Guardamos matriz model atual na pilha
                    model = model * Matrix_Translate(0.0f, -0.75f, 0.0f); // Atualizamos matriz model (multiplicação à esquerda) com a translação da canela esquerda
                    PushMatrix(model); // Guardamos matriz model atual na pilha
                        model = model * Matrix_Scale(0.25f, 0.7f, 0.25f); // Atualizamos matriz model (multiplicação à esquerda) com um escalamento da canela esquerda
                        glUniformMatrix4fv(model_uniform, 1, GL_FALSE, glm::value_ptr(model)); // Enviamos matriz model atual para a GPU
                        DrawCube(render_as_black_uniform); // #### CANELA ESQUERDA // Desenhamos a canela esquerda
                        PushMatrix(model); // Guardamos matriz model atual na pilha
                            model = model * Matrix_Translate(0.0f, -1.08f, 0.5f); // Atualizamos matriz model (multiplicação à esquerda) com a translação do pé esquerdo
                            PushMatrix(model); // Guardamos matriz model atual na pilha
                                model = model * Matrix_Scale(0.8f, 0.15f, 2.1f); // Atualizamos matriz model (multiplicação à esquerda) com um escalamento do pé esquerdo
                                glUniformMatrix4fv(model_uniform, 1, GL_FALSE, glm::value_ptr(model)); // Enviamos matriz model atual para a GPU
        ...
        // ===========================
        PushMatrix(model); // Guardamos matriz model atual na pilha
            model = model * Matrix_Translate(0.0f, 0.05f, 0.0f); // Atualizamos matriz model (multiplicação à direita) com uma translação para o braço direito
            PushMatrix(model); // Guardamos matriz model atual na pilha
                model = model // Atualizamos matriz model (multiplicação à direita) com a rotação do braço direito
                      * Matrix_Rotate_Z(g_AngleZ)  // TERCEIRO rotação Z de Euler
                      * Matrix_Rotate_Y(-g_AngleY)  // SEGUNDO rotação Y de Euler
                      * Matrix_Rotate_X(-g_AngleX); // PRIMEIRO rotação X de Euler
                PushMatrix(model); // Guardamos matriz model atual na pilha
                    model = model * Matrix_Scale(0.35f, -0.35f, 0.35f); // Atualizamos matriz model (multiplicação à direita) com um escalamento do braço direito
                    glUniformMatrix4fv(model_uniform, 1, GL_FALSE, glm::value_ptr(model)); // Enviamos matriz model atual para a GPU
                    DrawCube(render_as_black_uniform); // #### BRAÇO DIREITO // Desenhamos o braço direito
                PopMatrix(model); // Tiramos da pilha a matriz model guardada anteriormente
            PopMatrix(model); // Tiramos da pilha a matriz model guardada anteriormente
