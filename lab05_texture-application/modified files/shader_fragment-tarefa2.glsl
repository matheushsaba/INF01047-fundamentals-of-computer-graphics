...
void main()
{
    ...
    vec3 Kd1 = texture(TextureImage1, vec2(U,V)).rgb;    // Leitura da textura de luzes do planeta

    // Equação de Iluminação
    float lambert = max(0,dot(n,l));

    // color.rgb = Kd0 * (lambert + 0.01);
    color.rgb = Kd0 * (lambert + 0.01) + Kd1 * (1 - lambert);   // Adição de parcela da textura de luzes no cálculo de cores
